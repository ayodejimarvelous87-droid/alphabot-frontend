  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!res.ok) {
          setMessage(
            data.message || "Unable to load ePIN prices"
          );
          return;
        }

        const epinProducts = data.filter(
          (item) =>
            String(item.category || "").toLowerCase() ===
            "epin"
        );

        setProducts(epinProducts);

        const initialPrices = {};

        epinProducts.forEach((item) => {
        ...
        {products.length === 0 ? (
          ...
          ">
            ...
            <h2 className="text-xl font-bold mt-4">
              No ePIN products found
            </h2>

            <p className="text-zinc-500 text-sm mt-2">
              No active ePIN products are currently
              available.
            </p>
          </div>
        ) : (
