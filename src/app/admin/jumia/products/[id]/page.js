...
export default function EditJumiaProductPage() {
  ...
  const [form, setForm] = useState({ category: "", markup: "", weight: "", length: "", width: "", height: "", deliveryDaysFrom: "", deliveryDaysTo: "" });
    ...
    const loadProduct = async () => {
      try {
        ...
        setForm({
          ...
          deliveryDaysFrom: item.deliveryDaysFrom ?? item.deliveryDays?.from ?? item.deliveryDays ?? "",
          deliveryDaysTo: item.deliveryDaysTo ?? item.deliveryDays?.to ?? item.deliveryDays?.from ?? item.deliveryDays ?? ""
        });
